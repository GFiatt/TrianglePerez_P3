...
public final class Checker implements Visitor {
  ...
  public Object visitCaseCommand(CaseCommand ast, Object o) {
    ...
    // Validar el tipo del Vname (debe ser Integer o Character)
    ast.V.visit(this, null);
    if (ast.V.type != StdEnvironment.integerType && ast.V.type != StdEnvironment.charType) {
      reporter.reportError("Case Vname must be of type Integer or Character", "", ast.position);
    }

    // Validar las claves y los comandos asociados
    HashSet<String> usedKeys = new HashSet<>();
    for (Object key : ast.MAP.keySet()) {
      String keySpelling = (key instanceof IntegerLiteral)
              ? ((IntegerLiteral) key).spelling
              : ((CharacterLiteral) key).spelling;

      if (usedKeys.contains(keySpelling)) {
        reporter.reportError("Duplicate case key: " + keySpelling, "", ast.position);
      } else {
        usedKeys.add(keySpelling);
      }

      ast.MAP.get(key).visit(this, o);
    }

    // Validar el comando por defecto (else), si existe
    if (ast.C != null) {
      ast.C.visit(this, o);
    }
  ...
  public Object visitRepeatCommand(RepeatCommand ast, Object o) {
    ast.C.visit(this, null); // Verifica el comando dentro del repeat
    TypeDenoter eType = (TypeDenoter) ast.E.visit(this, null); // Verifica la expresión hasta que sea válida
    if (!eType.equals(StdEnvironment.booleanType)) {
      reporter.reportError("Repeat expression must be of type Boolean", "", ast.E.position);
    }
    return null;
  }
  ...
  public Object visitDoCommand(DoCommand ast, Object o) {
    // Verificar el contexto del comando dentro del bloque `do`
    ast.C.visit(this, o);

    // Verificar que la expresión sea de tipo booleano
    TypeDenoter expressionType = (TypeDenoter) ast.E.visit(this, o);
    if (!expressionType.equals(StdEnvironment.booleanType)) {
      reporter.reportError("The expression in a do-while must be of type Boolean", "", ast.E.position);
    }
  ...
  public Object visitMethodCallExpression(MethodCallExpression ast, Object o) {
    TypeDenoter recordType = (TypeDenoter) ast.recordVname.visit(this, null);
    if (!(recordType instanceof RecordTypeDenoter)) {
      reporter.reportError("The variable is not a record type", "", ast.position);
      return null;
    }

    RecordTypeDenoter record = (RecordTypeDenoter) recordType;
    Declaration methodDecl = record.findMethod(ast.methodName);

    if (methodDecl == null) {
      reporter.reportError("The method \"" + ast.methodName.spelling + "\" is not defined in the record", "", ast.position);
      return null;
    }

    if (methodDecl instanceof FuncDeclaration) {
      FuncDeclaration funcDecl = (FuncDeclaration) methodDecl;
      FormalParameterSequence formalParams = funcDecl.FPS;

      /* validacion de cant de parametros. no es necesario
      if (formalParams.size() != ast.parameterSequence.size()) {
        reporter.reportError("Parameter count mismatch", "", ast.position);
      }*/
    }
  ...
  public Object visitCaseExpression(CaseExpression ast, Object o) {
    ...
    if (!ast.V.type.equals(StdEnvironment.integerType) && !ast.V.type.equals(StdEnvironment.charType)) {
      reporter.reportError("Case expression must be of type Integer or Character", "", ast.position);
    }

    for (Object literal : ast.MAP.keySet()) {
      if (literal instanceof IntegerLiteral && !ast.V.type.equals(StdEnvironment.integerType)) {
        reporter.reportError("Integer literal does not match case expression type", "", ast.position);
      } else if (literal instanceof CharacterLiteral && !ast.V.type.equals(StdEnvironment.charType)) {
        reporter.reportError("Character literal does not match case expression type", "", ast.position);
      } else if (!(literal instanceof IntegerLiteral || literal instanceof CharacterLiteral)) {
        reporter.reportError("Invalid literal type in case expression", "", ast.position);
      }

      ast.MAP.get(literal).visit(this, null);
    }

    return ast.type = StdEnvironment.integerType; // Assuming integer result for simplicity.
  }
