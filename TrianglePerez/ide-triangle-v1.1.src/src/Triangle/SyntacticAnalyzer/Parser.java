  ...
  Command parseSingleCommand() throws SyntaxError {
    ...
    switch (currentToken.kind) {
      ...
      case Token.FOR:
        ...
        Expression byValue = null;

        if (currentToken.kind == Token.BY) {
          acceptIt();
          if (currentToken.kind == Token.OPERATOR && currentToken.spelling.equals("-")) {
            String negativeSign = currentToken.spelling;
            acceptIt();
            if (currentToken.kind == Token.INTLITERAL) {
              byValue = new IntegerExpression(
                      new IntegerLiteral(negativeSign + currentToken.spelling, currentToken.position),
                      currentToken.position
              );
              acceptIt();
            } else {
              syntacticError("Integer literal expected after '-'", currentToken.spelling);
            }
          } else if (currentToken.kind == Token.INTLITERAL) {
            byValue = new IntegerExpression(
                    new IntegerLiteral(currentToken.spelling, currentToken.position),
                    currentToken.position
            );
            acceptIt();
          } else {
            syntacticError("Integer literal expected after 'by'", currentToken.spelling);
          }
        }
        ...
        if (currentToken.kind == Token.DO) {
          acceptIt();
          Command cAST = parseSingleCommand();
          commandAST = new ForCommand(vAST, e1AST, e2AST, byValue, cAST, commandPos);
        } else {
          syntacticError("\"do\" expected here", currentToken.spelling);
        }
        break;
      ...
      case Token.DO:
        ...
        commandAST = new DoCommand(commandAST, expressionAST, commandPos);
        break;
      ...
      case Token.CASE: {
        ...
        vAST = parseVname(); // Usamos Vname como lo especifica la tarea
        accept(Token.OF);

        LinkedHashMap<Object, Command> map = new LinkedHashMap<>();
        while (currentToken.kind == Token.INTLITERAL || currentToken.kind == Token.CHARLITERAL) {
          Object literal;
          if (currentToken.kind == Token.INTLITERAL) {
            literal = parseIntegerLiteral();
          } else {
            literal = parseCharacterLiteral();
          }
          accept(Token.COLON);
          Command command = parseSingleCommand();
          map.put(literal, command);
        ...
        if (currentToken.kind == Token.ELSE) {
          acceptIt();
          accept(Token.COLON);
          elseCommand = parseSingleCommand();
        }

        accept(Token.END);
        finish(casePos);

        commandAST = new CaseCommand(vAST, map, elseCommand, casePos);
      }
      break;
      ...
      case Token.REPEAT:
        acceptIt();
        Command repeatCommandAST = parseCommand();
        accept(Token.UNTIL);
        Expression repeatExpressionAST = parseExpression();
        commandAST = new RepeatCommand(repeatCommandAST, repeatExpressionAST, commandPos);
        break;
  ...
  Expression parseExpression() throws SyntaxError {
    ...
    switch (currentToken.kind) {
      ...
      case Token.CASE:
      {
        ...
        Vname vAST = parseVname();
        accept(Token.OF);
        LinkedHashMap<Object, Expression> map = new LinkedHashMap<>();
